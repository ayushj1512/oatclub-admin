...
/* ---------------------------------------------
   ...
   - ✅ Backend: fulfillmentStatus=shipped + customerName=search
--------------------------------------------- */
export default function ShippedOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      console.log("Shipped Orders Fetch Error:", e);
    }
  ...
  const filteredOrders = useMemo(() => {
    ...
    data = data.filter(
      (o) => String(o?.fulfillmentStatus || "").toLowerCase() === "shipped"
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        pdf.save(`Shipped-Invoices-${Date.now()}.pdf`);
  ...
  const buildCsvRows = (ordersArr) => {
    ...
    for (const order of ordersArr || []) {
      ...
      // shipment info (nice for shipped)
      const awb =
        safe(order?.shipment?.shiprocket?.awb) ||
        safe(order?.shipment?.xpressbees?.awb) ||
  ...
  const exportToCSV = () => {
    if (!filteredOrders?.length) return alert("No shipped orders to export.");
    ...
    link.href = url;
    link.setAttribute("download", `shipped-orders-${ts}.csv`);
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Search and manage only <b>shipped</b> orders.
            </p>
            <div className="mt-4 flex items-center gap-3 text-sm text-gray-600">
              ...
              <span className="px-3 py-1 rounded-full bg-sky-50 text-sky-800 font-semibold">
                Status: shipped
              </span>
            </div>
          </div>
                ...
                <tr>
                  <th className="w-12 px-4 py-4 text-left">
                    <input
                      ...
                      aria-label="Select all visible shipped orders"
