...
/* ---------------------------------------------
   ...
   - ✅ Backend: fulfillmentStatus=returned + customerName=search
--------------------------------------------- */
export default function ReturnedOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      console.log("Returned Orders Fetch Error:", e);
    }
  ...
  const filteredOrders = useMemo(() => {
    ...
    // Safety: keep only returned
    data = data.filter(
      (o) => String(o?.fulfillmentStatus || "").toLowerCase() === "returned"
    );
  ...
  /* ---------------------------------------------
     ✅ CSV export
     - includes latest RMA + reverse shipment info (helpful in returned)
  --------------------------------------------- */
  const buildCsvRows = (ordersArr) => {
    ...
    for (const order of ordersArr || []) {
      ...
      const pickedAt = formatDateISO(latestRma?.reverseShipment?.pickedAt || "");
      const receivedAt = formatDateISO(latestRma?.reverseShipment?.receivedAt || "");
  ...
  const exportToCSV = () => {
    if (!filteredOrders?.length) return alert("No returned orders to export.");
    ...
    link.href = url;
    link.setAttribute("download", `returned-orders-${ts}.csv`);
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Search and manage <b>returned</b> orders.
            </p>
