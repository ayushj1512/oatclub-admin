...
const getLatestRma = (order) => {
  const rmas = Array.isArray(order?.rmas) ? order.rmas : [];
  return rmas.length ? rmas[rmas.length - 1] : null;
};

const getOrderDate = (order) => {
  const latestRma = getLatestRma(order);
...
export default function ExchangedOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      console.log("Exchanged Orders Fetch Error:", e);
    }
  ...
  const filteredOrders = useMemo(() => {
    ...
    data = data.filter(
      (o) => String(o?.fulfillmentStatus || "").toLowerCase() === "exchanged"
    );
    ...
    if (q) {
      data = data.filter((o) => {
        ...
        const latestRma = getLatestRma(o);
        ...
        const feeStatus = String(latestRma?.fee?.status || "").toLowerCase();
  ...
  const withRmaCount = useMemo(() => {
    return sortedOrders.filter((o) => !!getLatestRma(o)).length;
  }, [sortedOrders]);

  const feePaidCount = useMemo(() => {
    return sortedOrders.filter((o) => {
      const latestRma = getLatestRma(o);
      return String(latestRma?.fee?.status || "").toLowerCase() === "paid";
    }).length;
  }, [sortedOrders]);
  ...
  const buildCsvRows = (ordersArr) => {
    ...
    for (const order of ordersArr || []) {
      ...
      const feeAmount = money(latestRma?.fee?.amount);
      const feeStatus = safe(latestRma?.fee?.status || "");
      const feeCurrency = safe(latestRma?.fee?.currency || "");
  ...
  const exportToCSV = () => {
    if (!sortedOrders.length) {
      alert("No exchanged orders to export.");
      return;
    }
    ...
    link.href = url;
    link.setAttribute("download", `exchanged-orders-${ts}.csv`);
      ...
      <div className="mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Search and manage <b>exchanged</b> orders.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">Exchanged Orders</p>
          ...
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">{feePaidCount}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <Card>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <div className="md:col-span-2 flex items-center gap-3 bg-white rounded-2xl px-4 py-3 border border-gray-100">
              ...
              <input
                type="text"
                placeholder="Search 000123 / name / email / phone / RMA..."
                ...
                {sortedOrders.length ? (
                  ...
                  <tr>
                    <td colSpan={7} className="py-12 text-center text-gray-500">
                      {loading ? "Loading exchanged orders..." : "No exchanged orders found."}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <Card className="p-4">
          <div className="text-xs text-gray-500 flex flex-wrap gap-4">
            <span>
              Reverse AWB Added: <b className="text-gray-700">{reverseAwbCount}</b>
            </span>
            <span>
              Fee Paid Exchanges: <b className="text-gray-700">{feePaidCount}</b>
            </span>
          </div>
        </Card>
      </div>
