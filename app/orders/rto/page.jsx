...
/* ---------------------------------------------
   ...
   - ✅ Backend: fulfillmentStatus=rto + customerName=search
--------------------------------------------- */
export default function RtoOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      console.log("RTO Orders Fetch Error:", e);
    }
  ...
  const filteredOrders = useMemo(() => {
    ...
    // Safety: keep only rto
    data = data.filter((o) => String(o?.fulfillmentStatus || "").toLowerCase() === "rto");
  ...
  /* ---------------------------------------------
     ✅ CSV export
     - includes forward shipment details (Shiprocket/Xpressbees)
  --------------------------------------------- */
  const buildCsvRows = (ordersArr) => {
    ...
    for (const order of ordersArr || []) {
      ...
      // Forward shipment details (Shiprocket + Xpressbees)
      const shipProvider = safe(order?.shipment?.provider || "");
      const srAwb = safe(order?.shipment?.shiprocket?.awb || "");
      const srShipmentId = safe(order?.shipment?.shiprocket?.shipmentId || "");
      const srCourier = safe(order?.shipment?.shiprocket?.courierName || "");
      const srTrackingUrl = safe(order?.shipment?.shiprocket?.trackingUrl || "");

      const xbAwb = safe(order?.shipment?.xpressbees?.awb || "");
      const xbShipmentId = safe(order?.shipment?.xpressbees?.shipmentId || "");
      const xbCourier = safe(order?.shipment?.xpressbees?.courierName || "");
      const xbTrackingUrl = safe(order?.shipment?.xpressbees?.trackingUrl || "");

      const shippedAt = formatDateISO(order?.shipment?.shippedAt || order?.trackingDetails?.shippedAt);
      const deliveredAt = formatDateISO(order?.shipment?.deliveredAt || order?.trackingDetails?.deliveredAt);
      ...
      if (!items.length) {
        rows.push({
          ...
          shiprocketCourier: srCourier,
          shiprocketTrackingUrl: srTrackingUrl,
          xpressbeesAwb: xbAwb,
          xpressbeesShipmentId: xbShipmentId,
          xpressbeesCourier: xbCourier,
          xpressbeesTrackingUrl: xbTrackingUrl,
          shippedAt,
          deliveredAt,
          subtotal,
          discount,
  ...
  const exportToCSV = () => {
    if (!filteredOrders?.length) return alert("No RTO orders to export.");
    ...
    const headers = [
      ...
      "Xpressbees AWB",
      "Xpressbees ShipmentId",
      "Xpressbees Courier",
      "Xpressbees Tracking URL",
      "Shipped At (ISO)",
      "Delivered At (ISO)",
      "Subtotal",
      "Discount",
    ...
    const csvLines = [
      ...
      ...rows.map((r) =>
        [
          ...
          r.shiprocketCourier,
          r.shiprocketTrackingUrl,
          r.xpressbeesAwb,
          r.xpressbeesShipmentId,
          r.xpressbeesCourier,
          r.xpressbeesTrackingUrl,
          r.shippedAt,
          r.deliveredAt,
          r.subtotal,
          r.discount,
    ...
    link.href = url;
    link.setAttribute("download", `rto-orders-${ts}.csv`);
    ...
    <section className="min-h-screen bg-[#f6f7fb] px-4 sm:px-6 lg:px-10 py-10">
      <div className="mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gray-900">RTO Orders</h1>
            <p className="text-gray-500 mt-1">
              Search and manage <b>RTO</b> orders.
            </p>
            <div className="mt-4 flex items-center gap-3 text-sm text-gray-600">
              ...
              <span className="px-3 py-1 rounded-full bg-orange-50 text-orange-800 font-semibold">
                Status: rto
              </span>
            </div>
          </div>
