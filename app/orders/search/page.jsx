...
function Row({ label, value, mono = false }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-zinc-100 py-2 last:border-b-0">
      ...
      <div className={cn("text-right text-sm text-zinc-900", mono && "font-mono")}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <div className="relative">
      <select
        ...
        className="w-full appearance-none bg-white px-3 py-2 text-sm outline-none ring-1 ring-zinc-200/70 focus:ring-zinc-400"
      >
        {options.map((op) => (
...
function Img({ src, alt }) {
  ...
  if (!src || bad) {
    return (
      <div className="flex h-16 w-14 items-center justify-center bg-zinc-100 text-[10px] text-zinc-500 ring-1 ring-zinc-200/60">
        NO IMG
      </div>
    );
  }
  ...
  return (
    <img
      ...
      alt={alt}
      onError={() => setBad(true)}
      className="h-16 w-14 bg-zinc-100 object-cover ring-1 ring-zinc-200/60"
      loading="lazy"
    />
  );
}
...
export default function OrderSearchPage() {
  ...
  const search = useCallback(async () => {
    ...
    if (!ord) return toast.error("Enter valid order number");
  ...
  }, [order?.orderNumber, fetchOrderByNumber, syncNextFromOrder]);
  ...
  }, [order, nextFulfillment, updateOrderStatus, syncNextFromOrder, openCancelModal]);
  ...
  const handleCancelConfirm = useCallback(
    async (reason = "") => {
      ...
      setActionBusy(true);
      try {
        const o = await confirmCancel(reason);
        setOrder(o || { ...order, fulfillmentStatus: "cancelled" });
        syncNextFromOrder(o || { ...order, fulfillmentStatus: "cancelled" });
    ...
    [order, confirmCancel, syncNextFromOrder]
  );
  ...
  }, [order?._id, bookShiprocketIfMissing, refresh]);
  ...
  const syncTrackingNow = useCallback(async () => {
    ...
    try {
      await syncTracking(
        order?._id ? { orderId: order._id } : { orderNumber: order.orderNumber }
      );
    ...
    <>
      <div className="min-h-screen bg-zinc-50">
        <div className="space-y-4 px-4 py-6">
            ...
            <div className="px-4 py-4 md:py-5">
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={search}
                  disabled={loading}
                  className="inline-flex items-center gap-2 bg-zinc-900 px-4 py-3 text-sm text-white hover:bg-black disabled:opacity-60"
                >
                  {loading ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
          ...
          {!order?._id ? (
            <div className="bg-white p-6 shadow-sm ring-1 ring-zinc-200/60">
              <div className="text-sm font-semibold text-zinc-900">
                No order loaded
              </div>
                ...
                <Card title={`Items (${items.length})`} icon={Package} accent="info">
                  <div className="space-y-3">
                    {items.map((it, idx) => {
                      ...
                      const img =
                        safe(snap?.thumbnail) ||
                        (Array.isArray(snap?.images) ? safe(snap.images[0]) : "");
                      ...
                      return (
                        <div
                          ...
                          className="flex gap-3 border-b border-zinc-100 py-3 last:border-b-0"
                        >
                ...
                <Card title="Actions" icon={ShieldCheck} accent="success">
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <Btn
                        onClick={doConfirm}
                        disabled={!canConfirm || actionBusy || cancelLoading}
                      >
                        {actionBusy ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
                      ...
                      <Btn
                        onClick={markPacked}
                        disabled={!canPack || actionBusy || cancelLoading}
                        variant="indigo"
                      >
                        {actionBusy ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                    ...
                    <div className="space-y-2 bg-zinc-50 p-3 ring-1 ring-zinc-200/60">
                      ...
                      <Btn
                        onClick={applyStatus}
                        disabled={actionBusy || cancelLoading || !order?._id}
                      >
                        {actionBusy || cancelLoading ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : null}
                        Apply Status
                      </Btn>
      ...
      <CancelOrderModal
        ...
        loading={cancelLoading || actionBusy}
        onClose={() => {
          closeCancelModal();
          syncNextFromOrder(order);
        }}
