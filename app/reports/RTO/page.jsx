...
import { FileSpreadsheet, RefreshCw, Search, Loader2, Download } from "lucide-react";
...
const buildExcelRows = (orders = [], reportType = "") =>
  orders.map((o, idx) => {
...
export default function RTOReportsPage() {
  ...
  const [reportType, setReportType] = useState("rto"); // rto | cancelled
  ...
  const currentStatus = reportType === "rto" ? "rto" : "cancelled";
  const reportLabel = reportType === "rto" ? "RTO Report" : "Cancellation Report";
  ...
  }, [appliedSearch, currentStatus, endDate, fetchAllOrders, paymentMethod, startDate]);
  ...
  const summary = useMemo(() => {
    ...
    const totalValue = list.reduce((sum, o) => sum + Number(o.finalPayable || 0), 0);
    const codCount = list.filter((o) => String(o.paymentMethod || "").toLowerCase() === "cod").length;
    const prepaidCount = list.filter(
      (o) => String(o.paymentMethod || "").toLowerCase() === "razorpay"
    ).length;

    return { totalOrders, totalValue, codCount, prepaidCount };
  }, [orders]);
  ...
  const handleExport = async () => {
    try {
      ...
      const rows = buildExcelRows(
        orders || [],
        reportType === "rto" ? "RTO" : "Cancellation"
      );

      const workbook = new ExcelJS.Workbook();
      const sheet = workbook.addWorksheet(reportType === "rto" ? "RTO Report" : "Cancellation Report");
      ...
      const fileName = `${reportType === "rto" ? "RTO" : "Cancellation"}_Report_${startDate || "all"}_${endDate || "all"}.xlsx`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 rounded-3xl bg-white p-5 shadow-sm border border-gray-200 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              View and export RTO and cancellation orders.
            </p>
          </div>

          <div className="flex flex-wrap gap-2">
            <Button
              variant={reportType === "rto" ? "default" : "outline"}
              onClick={() => setReportType("rto")}
            >
              RTO Report
            </Button>

            <Button
              variant={reportType === "cancelled" ? "default" : "outline"}
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <Card className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">{summary.codCount}</p>
          </Card>
          ...
          <Card className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">{summary.prepaidCount}</p>
          </Card>
        </div>
        ...
        <Card className="p-4 md:p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold">{reportLabel}</h2>
              <p className="text-sm text-gray-600">
                Filter by date, payment method, and search.
              </p>
            </div>
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-gray-200 px-4 py-3 md:px-5">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className={statusBadgeClasses(currentStatus)}>
                {reportType === "rto" ? "RTO Orders" : "Cancelled Orders"}
              </Badge>

              {ordersMeta?.totalCount != null && (
                <Badge className="border-gray-200 bg-gray-50 text-gray-700">
                  Count: {ordersMeta.totalCount}
                </Badge>
              )}
                      ...
                      <tr key={o._id} className="align-top hover:bg-gray-50/70">
                        ...
                        <td className="border-b border-gray-100 px-4 py-4">
                          <div className="flex flex-col gap-2">
                            ...
                            <Badge className={statusBadgeClasses(shipment.status)}>
                        ...
                        <td className="border-b border-gray-100 px-4 py-4">
                          <div className="max-w-[260px] text-xs text-gray-600 space-y-1">
                            <div>
