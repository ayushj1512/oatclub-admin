...
export default function PromotionCreditsPage() {
  const {
    allCreditLogs,
    allCreditLogsTotal,
    allCreditLogsPage,
  ...
  const fetchPromotions = async (next = filters) => {
    const res = await fetchAllCustomerCreditLogs(next);
    if (res?.success === false) {
      toast.error(res.error || "Failed to load promotions");
    }
  };
  ...
  const applyFilters = () => fetchPromotions({ ...filters, page: 1 });

  const changePage = (page) => {
    const next = { ...filters, page };
    setFilters(next);
    fetchPromotions(next);
  };
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 md:px-6">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Campaign, coupon and promotional wallet credit history.
          </p>
        </div>

        <button
          onClick={() => fetchPromotions(filters)}
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2">
        <StatCard
          icon={Megaphone}
          label="Matched Promotion Logs"
          value={allCreditLogsTotal || 0}
        />
        <StatCard
          icon={Gift}
          label="Promotion Amount On Page"
      ...
      <section className="card mb-6 p-4">
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          <Input
            ...
            placeholder="Search customer, campaign, reason..."
          />
          ...
          <Input
            ...
            placeholder="Coupon code e.g. SUMMER500"
          />
      ...
      <section className="table-container">
        <div className="border-b border-[var(--primary-border)] p-5">
          <h2 className="section-title">Promotion Credit History</h2>
          <p className="mt-1 text-sm text-gray-500">
                ...
                allCreditLogs.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        {log.couponCode ? (
                          <span className="badge-primary">{log.couponCode}</span>
                        ) : (
        ...
        <div className="flex items-center justify-between border-t border-[var(--primary-border)] p-4">
          ...
          <span className="text-sm text-gray-500">
            {allCreditLogsTotal || 0} promotion logs
          </span>
