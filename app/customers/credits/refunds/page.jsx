...
export default function RefundCreditsPage() {
  const {
    allCreditLogs,
    allCreditLogsTotal,
    allCreditLogsPage,
  ...
  const fetchRefunds = async (next = filters) => {
    const res = await fetchAllCustomerCreditLogs(next);
    if (res?.success === false) toast.error(res.error || "Failed to load refunds");
  };
  ...
  const applyFilters = () => fetchRefunds({ ...filters, page: 1 });
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 md:px-6">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Wallet credits issued against refunds and order adjustments.
          </p>
        </div>
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2">
        <StatCard
          icon={ReceiptText}
          label="Matched Refund Logs"
          value={allCreditLogsTotal || 0}
        />
        <StatCard
          icon={Wallet}
          label="Refund Amount On Page"
      ...
      <section className="card mb-6 p-4">
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          <Input
            ...
            placeholder="Search customer, reason, refund note..."
          />
      ...
      <section className="table-container">
        <div className="border-b border-[var(--primary-border)] p-5">
          <h2 className="section-title">Refund Credit History</h2>
          <p className="mt-1 text-sm text-gray-500">
                ...
                allCreditLogs.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="max-w-[340px] px-5 py-4 text-gray-600">
