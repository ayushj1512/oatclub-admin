...
const CREDIT_TYPES = [
  ...
  { value: "influencer", label: "Influencer Credit" },
  { value: "goodwill", label: "Goodwill Credit" },
  { value: "cashback", label: "Cashback" },
  { value: "manual_credit", label: "Manual Credit" },
];

export default function AddCustomerCreditPage() {
  const { fetchCustomers, addCustomerCredit, loadingList, creditSaving } =
    useCustomerStore();
  ...
  const submitHandler = async (e) => {
    ...
    const res = await addCustomerCredit(selectedCustomer._id, {
      ...form,
      amount: Number(form.amount),
    });

    if (!res?.success) return toast.error(res?.error || "Credit add failed");
    ...
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 md:px-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Search customer by phone, email, or customer ID. Select customer, then add wallet credit.
        </p>
      </div>
      ...
      <section className="card p-5">
        <h2 className="section-title">2. Credit Details</h2>
        ...
        <form onSubmit={submitHandler} className="mt-5 space-y-5">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Input
              ...
              placeholder="Refund issued as wallet credit"
            />
          ...
          {form.type === "promotion" && (
            <div className="grid gap-5 md:grid-cols-2">
              <Input
                ...
                onChange={(v) => updateField("promotionName", v)}
                placeholder="Summer Wallet Bonus"
              />
          ...
          {form.type === "influencer" && (
            <div className="grid gap-5 md:grid-cols-2">
              <Input
                ...
                onChange={(v) => updateField("influencerName", v)}
                placeholder="Ayesha Khan"
              />

              <Input
                label="Influencer Code"
                value={form.influencerCode}
                onChange={(v) => updateField("influencerCode", v)}
                placeholder="AYESHA500"
              />
            </div>
          )}
