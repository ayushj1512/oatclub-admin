...
const pickImage = (p) => {
  const candidates = [p?.thumbnail, p?.image, ...(Array.isArray(p?.images) ? p.images : [])];
  return candidates.find((x) => typeof x === "string" && x.trim().length) || "";
};
...
async function fetchAllPages(url) {
  ...
  const totalPages =
    Number(first?.totalPages ?? first?.pages ?? first?.pagination?.totalPages ?? first?.meta?.pagination?.totalPages) ||
    null;
  const page0 = Number(first?.page ?? first?.pagination?.page ?? 1) || 1;

  if (!totalPages || totalPages <= 1) return firstList;

  const all = [...firstList];
  for (let page = page0 + 1; page <= totalPages; page++) {
    const pageUrl = withParams(url, { limit: 1000, perPage: 1000, page });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ gather product identifiers (code OR id)
        const ids = Array.from(
          new Set(
            list
              .flatMap((c) => (Array.isArray(c?.cartAdds) ? c.cartAdds : []))
              .map((x) => safe(x?.productCode || x?.productId || x?.product))
              .filter(Boolean)
          )
        );
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Customers who added products to cart.</p>
        </div>

        <div className="flex flex-wrap gap-2 text-xs text-gray-700">
          <span className="rounded-full bg-gray-100 px-3 py-1">
            Customers: <b className="text-gray-900">{loading ? "…" : totals.customerCount}</b>
          </span>
          <span className="rounded-full bg-gray-100 px-3 py-1">
            Total Adds: <b className="text-gray-900">{loading ? "…" : totals.totalAdds}</b>
          </span>
          <span className="rounded-full bg-gray-100 px-3 py-1">
            Products: <b className="text-gray-900">{loadingProducts ? "…" : productCount}</b>
          </span>
        </div>
      </div>

      {loading ? (
        <div className="rounded-lg bg-gray-50 p-4 text-sm text-gray-700">Loading…</div>
      ) : groups.length === 0 ? (
        <div className="rounded-lg bg-gray-50 p-4 text-sm text-gray-700">No cart adds found.</div>
      ) : (
        <div className="space-y-4">
          {groups.map((g) => {
            ...
            return (
              <div key={g.customerId || Math.random()} className="rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
                {/* header */}
                  ...
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-gray-600">
                        {email ? <span className="rounded-full bg-gray-100 px-2 py-0.5">{email}</span> : null}
                        {phone ? <span className="rounded-full bg-gray-100 px-2 py-0.5">{phone}</span> : null}
                      </div>
                    </div>

                    <div className="mt-2 sm:mt-0 text-right text-xs text-gray-600">
                      <div>
                        <span className="text-gray-900 font-semibold">{total}</span> adds
                      </div>
                      <div>
                    ...
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="py-2 text-left text-[11px] font-semibold text-gray-500">Product</th>
                          <th className="py-2 text-left text-[11px] font-semibold text-gray-500">Product Code</th>
                          <th className="py-2 text-left text-[11px] font-semibold text-gray-500">Size</th>
                          <th className="py-2 text-left text-[11px] font-semibold text-gray-500">Price</th>
                          <th className="py-2 text-left text-[11px] font-semibold text-gray-500">Last Added</th>
                        </tr>
                      </thead>
                      ...
                      <tbody>
                        {cartAddsSorted.map((it, idx) => {
                          ...
                          // ✅ NEW: size stored in customer.cartAdds
                          const size = safe(it?.size || it?.selectedSize || "-");
                            ...
                            >
                              <td className="py-3 pr-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                    <div className="truncate text-sm font-semibold text-gray-900">
                                      {loadingProducts && !p ? "Loading..." : title || code || pid || "-"}
                                    </div>
                                  </div>
                                </div>
                              </td>

                              <td className="py-3 pr-3 text-sm font-semibold text-gray-900">{code || "-"}</td>

                              <td className="py-3 pr-3 text-sm text-gray-800">{size || "-"}</td>

                              <td className="py-3 pr-3 text-sm text-gray-800">{loadingProducts && !p ? "-" : price}</td>

                              <td className="py-3 text-sm text-gray-700">{lastAdded}</td>
                            </tr>
                          );
                        })}
                      </tbody>
